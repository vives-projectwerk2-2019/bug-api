...
const httpids = async (dev_id, user_dongle_id) => {
  var url = process.env.KIOSK + `/api/session/${dev_id}/${user_dongle_id}`; //http://localhost:8000/api/session/
  try {
...
const httpaddons = async (data, user_dongle_id) => {
  var url = process.env.KIOSK + `/api/activate_dongle/${user_dongle_id}`;
  try {
    await fetch(url, {
      method: "POST",
      credentials: "same-origin",
